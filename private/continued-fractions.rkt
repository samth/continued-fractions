...
(module+ test
  ...
  (define-syntax (check-convergents stx)
    (syntax-parse stx
      [(_ term-constructor (val:number ...) (r:number ...))
       #'(parameterize ((precision +inf.0)
                        (consume-limit 500))
           (for ((v (list val ...))
                 (rational (list r ...)))
             (let ((ref (pull (rat rational))))
               (printf "checking (~a ~a): " 'term-constructor v)
               (check-equal? (time (pull (cfce1 (term-constructor v) '((1 0)(0 1))) (length ref)))
                             ref
                             (format "(~a ~a)" 'term-constructor v)))))]))
  (let ((sqrt-5 (sequence-append (list 2) (endless-values 4))))
    (let ((phi (cfce1 phi-terms '((1 0) (0 1))))
          (phi-calc (cfce1 sqrt-5 '((1 1)(0 2)))))
      (parameterize ((precision +inf.0))
...
(module+ test
  (parameterize ((precision +inf.0))
    (let ((pi-cf (cfce1 pi-terms '((1 0)(0 1))))
          (ref (pull (rat 1001262030186123788147677117546011454046175300701051137698775095496/318711602868696242700351405960501012107407621461407582086159833951))))
...
(define (cosine-terms x)
  ; derived from maclaurin expansion for cosine
  ; but jiggered with to yield integer arithmetic
  ; first by removing the fractional parts though multiplication down the fraction
  ; and then by writing x as p/q and doing the same to eliminate denominators
  (define x^2 (* x x))
  (define x-factor (sequence-map (λ(t) (/ x^2 (* t (add1 t)))) odds))
  #;(define numerators (sequence-append (list 1) x-factor))
  #;(define denominators (sequence-append (list 0 1)
                                          (sequence-map (λ(t) (- 1 t)) x-factor)))
  #;(make-general-cf denominators numerators)
...
(module+ test
  (check-convergents cosine-terms
...
(define (sine-terms x)
  ...
  #;(define x^2 (* x x))
  #;(define x-factor (sequence-map (λ(t) (/ x^2 (* t (add1 t)))) (sequence-tail evens 1)))
  #;(define numerators (sequence-append (list x) x-factor))
  #;(define denominators (sequence-append (list 0 1)
                                          (sequence-map (λ(t) (- 1 t)) x-factor)))
  #;(make-general-cf denominators numerators)
  (define-values (p q) (values (numerator x) (denominator x)))
...
(module+ test
  (check-convergents tangent-terms
...
(define (expt-terms base exponent)
...
(module+ test
  (check-convergents (λ(exp) (expt-terms 7933/17417 exp)) ; nth primes 1001 and 2002
